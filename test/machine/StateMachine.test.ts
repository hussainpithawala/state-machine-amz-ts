...
describe("StateMachine", () => {
    ...
    it("should throw on invalid JSON", () => {
        expect(() => StateMachine.fromJson("{invalid json}")).toThrow("Failed to parse JSON");
    });
    ...
    it("should throw on missing StartAt", () => {
      ...
      "States": { "FirstState": { "Type": "Pass", "End": true } }
        ...
        expect(() => StateMachine.fromJson(definition)).toThrow("StartAt is required");
    });

    it("should throw on missing States", () => {
        const definition = `{ "StartAt": "FirstState" }`;
        expect(() => StateMachine.fromJson(definition)).toThrow("States is required");
    });
    ...
    it("should capture execution metadata", async () => {
      ...
      "States": { "FirstState": { "Type": "Pass", "End": true } }
        ...
        expect(execCtx.endTime!.getTime()).toBeGreaterThanOrEqual(execCtx.startTime.getTime());
        expect(execCtx.currentState).toBe("FirstState");
    });
