...
describe("ParallelState", () => {
  ...
  it("should execute branches concurrently", async () => {
    ...
    class LoggingPassState extends PassState {
      ...
      constructor(
        name: string,
        delay: number,
        config: Partial<PassStateConfig>,
      ) {
        super({ name, ...config });
        this.delay = delay;
      }
    ...
    const state = new ParallelState({
      ...
      branches: [
        new Branch({
          startAt: "Pass1",
          states: {
            Pass1: new LoggingPassState("Pass1", 100, {
              result: "result1",
              end: true,
            }),
          },
        ...
        new Branch({
          startAt: "Pass2",
          states: {
            Pass2: new LoggingPassState("Pass2", 100, {
              result: "result2",
              end: true,
            }),
          },
    ...
    expect(Math.min(pass1StartIdx, pass2StartIdx)).toBeLessThan(
      Math.max(pass1EndIdx, pass2EndIdx),
    );
  });
  ...
  it("should validate parallel state configuration", () => {
    ...
    // No branches
    expect(
      () => new ParallelState({ name: "NoBranches", branches: [] }),
    ).toThrow("must have at least one branch");
    ...
    const state = new ParallelState({
      ...
      branches: [
        new Branch({
          startAt: "Error1",
          states: {
            Error1: new FailState({ name: "Error1", error: "Error1" }),
          },
        }),
        new Branch({
          startAt: "Pass1",
