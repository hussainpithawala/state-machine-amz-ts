...
const createMockHandler = (
    executeFunc: (resource: string, inputData: unknown, parameters?: Record<string, unknown>) => Promise<unknown> | unknown
): TaskHandler => {
    ...
    return {
        ...
        executeWithTimeout: async (resource, inputData, parameters, timeoutSeconds) => {
            if (timeoutSeconds && timeoutSeconds > 0) {
                const timeoutPromise = new Promise<never>((_, reject) => {
                    timeoutId = setTimeout(() => {
                        reject(new Error(`Task execution timed out after ${timeoutSeconds} seconds`));
                    }, timeoutSeconds * 1000);
                });
...
describe("TaskState", () => {
    it("should execute basic task state", async () => {
        const executeFunc = (_resource: string, inputData: unknown) => ({ result: "success", input: inputData });
    ...
    it("should execute task state with parameters", async () => {
        const executeFunc = (_resource: string, _inputData: unknown, parameters?: Record<string, unknown>) => ({ params: parameters });
        const state = new TaskState({
            name: "TaskState",
            resource: "arn:aws:lambda:us-east-1:123456789012:function:MyFunction",
            end: true,
    ...
    it("should execute task state with input path", async () => {
        ...
        const [output] = await state.execute({ data: { value: "test" }, other: "ignored" });
    ...
    it("should execute task state with retry that eventually succeeds", async () => {
        ...
        const state = new TaskState({
            ...
            retry: [new RetryRule({ errorEquals: ["TemporaryError"], intervalSeconds: 0, maxAttempts: 2, backoffRate: 1.0 })],
            taskHandler: createMockHandler(executeFunc),
        });
    ...
    it("should execute task state with retry that exhausts attempts", async () => {
        ...
        const state = new TaskState({
            ...
            retry: [new RetryRule({ errorEquals: ["PersistentError"], intervalSeconds: 0, maxAttempts: 2, backoffRate: 1.0 })],
            taskHandler: createMockHandler(executeFunc),
        });
    ...
    it("should execute task state with catch policy", async () => {
        const executeFunc = () => { throw new Error("CustomError"); };
        const state = new TaskState({
            name: "TaskState",
            resource: "arn:aws:lambda:us-east-1:123456789012:function:MyFunction",
            catch: [new CatchRule({ errorEquals: ["CustomError"], nextState: "ErrorHandler", resultPath: "$.error" })],
            taskHandler: createMockHandler(executeFunc),
            end: true,
        });
    ...
    it("should validate task state", () => {
        expect(() => new TaskState({ name: "TaskState", resource: "arn:aws:lambda:us-east-1:123456789012:function:MyFunction", end: true })).not.toThrow();
        expect(() => new TaskState({ name: "TaskState", resource: "", end: true })).toThrow("Resource is required");
        expect(() => new TaskState({ name: "TaskState2", resource: "arn:aws:lambda:us-east-1:123456789012:function:MyFunction", timeoutSeconds: 0, end: true })).toThrow("TimeoutSeconds must be positive");
        expect(() => new TaskState({ name: "TaskState", resource: "arn:aws:lambda:us-east-1:123456789012:function:MyFunction", heartbeatSeconds: -1, end: true })).toThrow("HeartbeatSeconds must be positive");
        expect(() => new TaskState({ name: "TaskState", resource: "arn:aws:lambda:us-east-1:123456789012:function:MyFunction", timeoutSeconds: 10, heartbeatSeconds: 10, end: true })).toThrow("HeartbeatSeconds must be less than TimeoutSeconds");
        expect(() => new TaskState({ name: "TaskState", resource: "arn:aws:lambda:us-east-1:123456789012:function:MyFunction", retry: [new RetryRule({ errorEquals: [] })], end: true })).toThrow("ErrorEquals cannot be empty");
        expect(() => new RetryRule({ errorEquals: ["Error"], backoffRate: 0.5 })).toThrow("BackoffRate must be >= 1.0");
        expect(() => new TaskState({ name: "TaskState", resource: "arn:aws:lambda:us-east-1:123456789012:function:MyFunction", catch: [new CatchRule({ errorEquals: ["Error"], nextState: "" })] })).toThrow("Next cannot be empty");
    });

    it("should validate task state getters (cannot have both Next and End)", () => {
        expect(() => new TaskState({ name: "MyTaskState", resource: "arn:aws:lambda:us-east-1:123456789012:function:MyFunction", end: true, nextState: "NextState" })).toThrow("State cannot have both Next and End");
    });

    it("should match errors correctly", () => {
        const state = new TaskState({ name: "TaskState", resource: "arn:aws:lambda:us-east-1:123456789012:function:MyFunction", end: true });
        ...
        expect((state as any).errorMatches(new Error("AnyError"), ["States.ALL"])).toBe(true);
        expect((state as any).errorMatches(new Error("UnhandledError"), ["CustomError"])).toBe(false);
    });
    ...
    it("should serialize task state to dict", () => {
        const state = new TaskState({
            ...
            catch: [new CatchRule({ errorEquals: ["Error"], nextState: "ErrorHandler" })],
        });
        const stateDict = state.toDict();
        expect(stateDict.Type).toBe("Task");
        expect(stateDict.Resource).toBe("arn:aws:lambda:us-east-1:123456789012:function:MyFunction");
