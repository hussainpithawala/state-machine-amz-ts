...
describe("FailState", () => {
    ...
    it("should create a FailState with minimal fields", () => {
        const state = new FailState({ name: "MinimalFail", error: "States.TaskFailed" });
    ...
    it("should enforce terminal state properties", () => {
        const state = new FailState({ name: "DefaultFail", error: "States.Failed" });
    ...
    it("should throw validation error with Next field", () => {
        expect(() =>
            new FailState({ name: "InvalidFail", error: "TestError", nextState: "NextState" })
        ).toThrow("cannot have Next field");
    });

    it("should throw validation error with End field", () => {
        expect(() => new FailState({ name: "InvalidFail", error: "TestError", end: true })).toThrow(
    ...
    it("should throw validation error with InputPath", () => {
        expect(() =>
            new FailState({ name: "InvalidFail", error: "TestError", inputPath: "$.data" })
        ).toThrow("cannot have InputPath");
    });

    it("should throw validation error with OutputPath", () => {
        expect(() =>
            new FailState({ name: "InvalidFail", error: "TestError", outputPath: "$.output" })
        ).toThrow("cannot have OutputPath");
    });

    it("should throw validation error with ResultPath", () => {
        expect(() =>
            new FailState({ name: "InvalidFail", error: "TestError", resultPath: "$.result" })
    ...
    it("should throw StateError with context", async () => {
        ...
        await expect(state.execute(sampleInputData, context)).rejects.toThrow(StateError);
    });
    ...
    it("should convert minimal FailState to dict", () => {
        const state = new FailState({ name: "MinimalFail", error: "States.Failed" });
    ...
    it("should convert FailState to JSON string", () => {
        ...
        expect(result).toEqual({ Type: "Fail", Error: "States.Failed", Cause: "JSON test" });
    });
    ...
    it("should throw StateError with different input types", async () => {
        ...
        const state = new FailState({ name: "TypeTestFail", error: "States.Failed" });
    ...
    it("should properly inherit from BaseState", () => {
        const state = new FailState({ name: "InheritanceTest", error: "TestError" });
    ...
    it("should handle AWS standard error codes", async () => {
        const awsErrors = [
            ...
            { code: "States.ResultPathMatchFailure", cause: "Result path match failed" },
