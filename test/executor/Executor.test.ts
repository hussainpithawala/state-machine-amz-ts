 ...
 */
import { BaseExecutor, ExecutionContextAdapter, StateRegistry, StateMachineInterface } from "../../src/executor/Executor";
...
describe("BaseExecutor", () => {
    ...
    it("should throw when stopping with null execution", async () => {
        const executor = new BaseExecutor();
        await expect(executor.stop(null as unknown as Execution)).rejects.toThrow("cannot be null or undefined");
    });
    ...
    it("should set execution to ABORTED when stopped", async () => {
        ...
        expect(executor.listExecutions().find(e => e.id === "exec-1")).toBeUndefined();
    });
    ...
    it("should remove completed executions after execute", async () => {
        ...
        expect(executor.listExecutions().find(e => e.id === execCtx.id)).toBeUndefined();
    });
});
