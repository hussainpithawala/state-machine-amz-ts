...
describe("StateFactory", () => {
    ...
    it("should initialize with default creators", () => {
        ...
        expect(() => factory.createState("Test", { Type: "Pass", End: true })).not.toThrow();
        expect(() => factory.createState("Test", { Type: "Fail", Error: "E" })).not.toThrow();
        expect(() => factory.createState("Test", { Type: "Succeed" })).not.toThrow();
        expect(() => factory.createState("Test", { Type: "Task", Resource: "R", End: true })).not.toThrow();
        expect(() => factory.createState("Test", { Type: "Choice", Choices: [], Default: "D" })).not.toThrow();
    });

    it("should create PassState", () => {
        const data = { Type: "Pass", Result: { foo: "bar" }, Next: "NextState", Comment: "Test comment" };
    ...
    it("should create FailState", () => {
        const data = { Type: "Fail", Error: "CustomError", Cause: "Something went wrong" };
    ...
    it("should create TaskState", () => {
        ...
        expect((state as TaskState).resource).toBe("arn:aws:lambda:us-east-1:123456789012:function:HelloWorld");
        expect((state as TaskState).retry).toHaveLength(1);
        expect((state as TaskState).retry[0].errorEquals).toEqual(["States.Timeout"]);
    ...
    it("should throw if Type is missing", () => {
        expect(() => factory.createState("NoType", {})).toThrow("missing Type field");
    });

    it("should throw if Type is unknown", () => {
        expect(() => factory.createState("Unknown", { Type: "Unknown" })).toThrow("Unknown state type: Unknown");
    });
