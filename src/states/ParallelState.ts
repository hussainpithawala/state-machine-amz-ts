...
export class ParallelState extends BaseState {
  ...
  override validate(options?: ValidateOptions): void {
    ...
    if (!this.branches || this.branches.length === 0) {
      throw new Error(
        `Parallel state '${this.name}' must have at least one branch`,
      );
    }
    ...
    for (let i = 0; i < this.branches.length; i++) {
      ...
      if (!branch.startAt) {
        throw new Error(
          `Parallel state '${this.name}' branch ${i}: StartAt is required`,
        );
      }
      if (!branch.states || Object.keys(branch.states).length === 0) {
        throw new Error(
          `Parallel state '${this.name}' branch ${i}: States must not be empty`,
        );
      }
  ...
  public static fromDict(
    ...
    stateFactory?: {
      createState: (name: string, data: Record<string, unknown>) => BaseState;
    },
  ): ParallelState {
    ...
    const resultSelector = stateDict.ResultSelector as
      | Record<string, unknown>
      | undefined;

    // Parse branches
    const branches: Branch[] = [];
    const branchesData = stateDict.Branches as
      | Record<string, unknown>[]
      | undefined;
    ...
    if (statesDict && typeof statesDict === "object") {
      if (stateFactory) {
        for (const [stateName, stateData] of Object.entries(statesDict)) {
          states[stateName] = stateFactory.createState(
            stateName,
        ...
        throw new Error(
          "StateFactory is required to parse branch states from dictionary.",
        );
      }
    }
