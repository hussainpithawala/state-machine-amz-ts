  ...
  private validateState(
    ...
    if (!terminalTypes.has(state.stateTypeAsString)) {
      if (
        ...
        !["Choice"].includes(state.type)
      ) {
        throw new Error(
          `State '${stateName}' must have either Next or End set`,
        );
      }
      ...
      if (state.isEnd() && state.getNext() !== undefined) {
        throw new Error(
          `State '${stateName}' cannot have both Next and End set`,
        );
      }
    }
  }
  ...
  private validateStateReferences(states: Record<string, BaseState>): void {
    for (const [stateName, state] of Object.entries(states)) {
      ...
      if (nextState !== undefined && !(nextState in states)) {
        throw new Error(
          `State '${stateName}' references non-existent state '${nextState}'`,
        );
      }
    ...
    if (state.stateTypeAsString === "Choice") {
      ...
      if (choiceState.default !== undefined && choiceState.default !== null) {
        if (!(choiceState.default in allStates)) {
          throw new Error(
            `Choice state '${stateName}' default '${choiceState.default}' not found`,
          );
        }
      }
